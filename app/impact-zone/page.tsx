        ...
        <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="rounded-xl p-4 border border-[var(--color-border)] bg-[color:rgba(255,255,255,0.02)]">
              ...
              {/* EnergyGraph expects a `series` prop: [{name, value}] */}
              <EnergyGraph
                series={[
                  { name: 'Impact', value: Number(impactDetails.impact_energy || 0) },
                  { name: 'Hiroshima', value: 63_000_000_000_000 } // 15 kt ~ 6.3e13 J placeholder
                ]}
              />
            </div>
            <div className="rounded-xl p-4 border border-[var(--color-border)] bg-[color:rgba(255,255,255,0.02)]">
              <h4 className="font-semibold">Impact 3D Visualization</h4>
              {/* Impact3D expects an impactPoint prop with lat/lon (or undefined) */}
              <Impact3D impactPoint={impactDetails && (impactDetails as any).impact_point ? { lat: (impactDetails as any).impact_point.lat, lon: (impactDetails as any).impact_point.lon } : undefined} />
            "use client"
